...
| `from_version` | `null` (default) | `v.1.1.0`
...
<details>
<summary><strong>опциональная AI-интеграция</strong></summary>
...
### Шаг 3.1: Запуск smoke test (30 секунд)
...
### Шаг 3.2: Проверка результата (30 секунд)

**✅ Критерии успеха:**

| Критерий | Ожидаемое значение |
|----------|-------------------|
| **Статус запуска** | `✅ Server started after Xs` |
| **HTTP Status** | `200` |
| **Response** | `{"status": "healthy", "service": "git-changelog-mcp"}` |
| **Итог** | `✅ Smoke test PASSED` |
...
### Проблема: MCP Inspector не подключается
...
docker restart demo-mcp

# 4. Обновить страницу Inspector (F5)
```
...
# 3. Перезапустить контейнер с правильным volume
...
| Сценарий | Время | Инструмент | Ключевой результат |
|----------|-------|------------|-------------------|
| **1. CHANGELOG** | 3 мин | `generate_changelog` | 3 версии, 17 коммитов, группировка |
| **2. Release Notes** | 4 мин | `generate_release_notes` | AI-улучшенные notes с migration guide |
| **3. Smoke Test** | 1 мин | `smoke` (CLI) | ✅ Health check passed |
| **Итого** | **8 мин** | **2+ tools** | **Все критерии выполнены** |

---

## ✅ Чек-лист готовности к демонстрации

Перед началом отсчёта времени убедиться:

- [ ] Docker образ собран: `docker images | grep git-changelog-mcp`
- [ ] Demo project создан: `ls demo_project/.git`
- [ ] Сервер запущен: `curl http://localhost:8000/health`
- [ ] MCP Inspector открыт в браузере
- [ ] Подключение настроено (Streamable HTTP, localhost:8000/mcp)
- [ ] Вкладка Tools активна, список инструментов загружен
- [ ] Терминал готов для smoke test
...
- [docs/images/](docs/images/) — Скриншоты интерфейса

---

**🏆 Демонстрация завершена!**

Все обязательные критерии хакатона выполнены:
- ✅ Минимум 1 рабочий сценарий (фактически 3)
- ✅ Воспроизводимо на demo_project/
- ✅ Показана польза 2+ tools (`generate_changelog`, `generate_release_notes`)
